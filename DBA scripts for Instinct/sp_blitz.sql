...
/****** Object:  StoredProcedure [dbo].[sp_Blitz]    Script Date: 31/07/2020 08:41:26 ******/
SET ANSI_NULLS ON
GO
					...
					IF @ProductVersionMajor >= 11 AND NOT EXISTS ( SELECT 1
						...
						IF NOT EXISTS ( SELECT  1
							...
							BEGIN
								...
								INSERT INTO #BlitzResults
									...
									URL,
									Details)
								SELECT 124, 150, 'Performance', 'Deadlocks Happening Daily', '.',
						...
						BEGIN
							...
							INSERT INTO #BlitzResults
								...
								URL,
								Details)
							SELECT TOP 1 125, 10, 'Performance', 'Plan Cache Erased Recently', '.',
						...
						IF NOT EXISTS ( SELECT  1
							...
							BEGIN
								...
								INSERT INTO #BlitzResults
									...
									URL,
									Details)
								SELECT 165, 50, 'Performance', 'Too Much Free Memory', '.',
						...
						*/
						...
						INSERT INTO #DatabaseDefaults
						  SELECT 'is_query_store_on', 0, 139, 210, 'Query Store Enabled', '.', NULL
						  FROM sys.all_columns 
...
IF @ProductVersionMajor >= 10
               AND NOT (@ProductVersionMajor = 10.5 AND @ProductVersionMinor < 4297) /* Skip due to crash bug: . */
			   AND NOT EXISTS ( SELECT  1
								FROM    #SkipChecks
				        ...
				        */
				        IF NOT EXISTS ( SELECT  1
					        ...
					        BEGIN
								...
								EXEC dbo.sp_MSforeachdb 'USE [?];  IF EXISTS (SELECT * FROM  sys.tables WITH (NOLOCK) WHERE name = ''sysmergepublications'' ) IF EXISTS ( SELECT * FROM sysmergepublications WITH (NOLOCK) WHERE retention = 0)   INSERT INTO #BlitzResults (CheckID, DatabaseName, Priority, FindingsGroup, Finding, URL, Details) SELECT DISTINCT 99, DB_NAME(), 110, ''Performance'', ''Infinite merge replication metadata retention period'', ''.'', (''The ['' + DB_NAME() + ''] database has merge replication metadata retention period set to infinite - this can be the case of significant performance issues.'')';
					        END;
				        /*
                        ...
                        */
						...
						IF NOT EXISTS ( SELECT  1
							...
							SET @StringToExecute = 'INSERT INTO #BlitzResults
													...
													''.'',
													(''SQL 2016 RTM has a bug involving dumps that happen every time Query Store cleanup jobs run. This is fixed in CU1 and later: .'')
					...
					BEGIN
						...
						INSERT  INTO #BlitzResults
								...
								SELECT  74 AS CheckID ,
										...
										CASE WHEN [T].[TraceFlag] = '834'  AND @ColumnStoreIndexesInUse = 1 THEN '.'
											 ELSE'.' END AS URL ,
										'Trace flag ' + 
			...
			*/
			...
			/* Add credits for the nice folks who put so much time into building and maintaining this for free: */
				...
				VALUES  ( -1 ,
						  ...
						  '.' ,
						  'To get help or add your own contributions, join us at ..'
